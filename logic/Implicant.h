...
public:
	 ...
	 */
	Implicant(const char *literal, unsigned char nVariables = 0)
			: Implicant(std::string(literal), nVariables)
	{}
	 ...
	 */
	std::string ToLiteral(unsigned char n = 0) const;

	// Setters

	void setForm(ullong x)
	{ form = x; }

	void setMask(ullong x)
	{ mask = x; }
...
typedef Implicant ElementaryDisjunction;

/**
 * Return a mask for an implicant of n variables,
 * such that all n variables appear in the implicant.
 *
 * @note If n=0 return a full mask for LONG_LONG_MAX variables.
 */
